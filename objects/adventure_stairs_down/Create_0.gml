px = x + 8;
py = y + 8;

pr = adventure_flowerFieldsf1;
switch(room)
{
	case adventure_flowerFieldsf1:
	pr = adventure_flowerFieldsf2;
	break;
	case adventure_flowerFieldsf2:
	pr = adventure_flowerFieldsf3;
	break;
	case adventure_flowerFieldsf3:
	pr = adventure_flowerFieldsf4;
	break;
	
	case adventure_willowWoodsf1:
	pr = adventure_willowWoodsf2;
	break;
	case adventure_willowWoodsf2:
	pr = adventure_willowWoodsf3;
	break;
	case adventure_willowWoodsf3:
	pr = adventure_willowWoodsf4;
	break;
	
	case adventure_autumnFallsf1:
	pr = adventure_autumnFallsf2;
	break;
	case adventure_autumnFallsf2:
	pr = adventure_autumnFallsf3;
	break;
	case adventure_autumnFallsf3:
	pr = adventure_autumnFallsf4;
	break;
	
	case adventure_snowflakePointf1:
	pr = adventure_snowflakePointf2;
	break;
	case adventure_snowflakePointf2:
	pr = adventure_snowflakePointf3;
	break;
	case adventure_snowflakePointf3:
	pr = adventure_snowflakePointf4;
	break;
	
	case adventure_sakuraStepsf1:
	pr = adventure_sakuraStepsf2;
	break;
	case adventure_sakuraStepsf2:
	pr = adventure_sakuraStepsf3;
	break;
	case adventure_sakuraStepsf3:
	pr = adventure_sakuraStepsf4;
	break;
	
	case adventure_feyTemplef1:
	pr = adventure_feyTemplef2;
	break;
	case adventure_feyTemplef2:
	pr = adventure_feyTemplef3;
	break;
	case adventure_feyTemplef3:
	pr = adventure_feyTemplef4;
	break;
	
	case adventure_ancientAbyssf1:
	pr = adventure_ancientAbyssf2;
	break;
	case adventure_ancientAbyssf2:
	pr = adventure_ancientAbyssf3;
	break;
	case adventure_ancientAbyssf3:
	pr = adventure_ancientAbyssf4;
	break;
	
	case adventure_endRainbowf1:
	pr = adventure_endRainbowf2;
	break;
	case adventure_endRainbowf2:
	pr = adventure_endRainbowf3;
	break;
	case adventure_endRainbowf3:
	pr = adventure_endRainbowf4;
	break;
	
	
}